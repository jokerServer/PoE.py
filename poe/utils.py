...
def unescape_to_list(props, ret_matches=False):
    ...
    if not has_table:
        ...
        prop_list = [
            x.replace('<em class="tc -corrupted">', "").replace("</em>", "")
            for x in prop_list
        ]
    else:
...
class ItemRender:
    def __init__(self, flavor):
        ...
        self.namebar_left = Image.open(
            f"{_dir}//{self.flavor}_namebar_left.png"
        ).convert("RGBA")
        self.namebar_right = Image.open(
            f"{_dir}//{self.flavor}_namebar_right.png"
        ).convert("RGBA")
        self.namebar_trans = Image.open(
            f"{_dir}//{self.flavor}_namebar_trans.png"
        ).convert("RGBA")
        self.separator = Image.open(f"{_dir}//{self.flavor}_separator.png").convert(
            "RGBA"
        )
        ...
        self.ascendancy_frame = Image.open(f"{_dir}//ascendancy_frame.png").convert(
            "RGBA"
        )
    ...
    def calc_size(self, stats, header):
        width = (
            self.header_font.getsize(header)[0] + (self.namebar_left.size[0] * 2) + 4
        )
        height = 0
        last_sep = False
        for stat in stats:
    ...
    def sort_stats(self, item):
        ...
        if not isinstance(item, PassiveSkill):
            if "weapon" in item.tags:
                ...
                if item.quality:
                    stats.append(
                        self.prop("Quality: ", f"+{item.quality}%", PROP_COLOR)
                    )
                if item.physical_damage:
                    stats.append(
                        self.prop("Physical Damage: ", item.physical_damage, PROP_COLOR)
                    )

                elements = {
                    element.split("_")[0]: getattr(item, element)
                    for element in ["fire_damage", "cold_damage", "lightning_damage"]
                ...
                if item.chaos_damage:
                    stats.append(
                        self.prop("Chaos Damage: ", item.chaos_damage, CHAOS_COLOR)
                    )
                if item.critical_chance:
                    stats.append(
                        self.prop(
                            "Critical Strike Chance: ", item.critical_chance, None
                        )
                    )
                if item.attack_speed:
                    stats.append(
                        self.prop("Attacks Per Second: ", item.attack_speed, PROP_COLOR)
                    )
                if int(item.range):
                ...
                if item.block:
                    stats.append(
                        self.prop("Chance To Block: ", f"{item.block}%", PROP_COLOR)
                    )
                if item.armour:
                ...
                if item.energy_shield:
                    stats.append(
                        self.prop("Energy Shield: ", item.energy_shield, PROP_COLOR)
                    )
                stats.append(separator)
                ...
                stats.append(
                    self.prop(item.gem_tags.replace(",", ", "), "", DESC_COLOR)
                )
                if item.stats_per_level[0]["mana multiplier"]:
                    stats.append(
                        self.prop(
                            "Mana Multiplier: ",
                            f"{item.stats_per_level[0]['mana multiplier']}%",
                            None,
                        )
                    )
                if item.radius:
                ...
                if not item.is_aura:
                    ...
                    stats.append(
                        self.prop(
                            "Mana Reserved: ",
                            f"{item.stats_per_level[0]['mana cost']}%",
                            None,
                        )
                    )
                ...
                try:
                    if item.stats_per_level[20]["stored uses"]:
                        stats.append(
                            self.prop(
                                "Stored Uses",
                                {item.stats_per_level[20]["stored uses"]},
                                None,
                            )
                        )
                except KeyError:
                    if item.stats_per_level[10]["stored uses"]:
                        stats.append(
                            self.prop(
                                "Stored Uses",
                                {item.stats_per_level[10]["stored uses"]},
                                None,
                            )
                        )
                ...
                if item.stats_per_level[0]["cooldown"]:
                    stats.append(
                        self.prop(
                            "Cooldown Time: ",
                            f"{item.stats_per_level[0]['cooldown']} sec",
                            None,
                        )
                    )
                if item.cast_time:
                    stats.append(
                        self.prop("Cast Time: ", f"{item.cast_time} sec", None)
                    )
                if item.stats_per_level[0]["critical strike chance"]:
                    stats.append(
                        self.prop(
                            "Critical Strike Chance: ",
                            f"{item.stats_per_level[0]['critical strike chance']}%",
                            None,
                        )
                    )
                if item.stats_per_level[0]["damage effectiveness"]:
                    stats.append(
                        self.prop(
                            ...
                            f"{item.stats_per_level[0]['damage effectiveness']}%",
                            None,
                        )
                    )
                stats.append(separator)
                ...
                if len(new_obj.split(" ")) > 7:
                    ...
                    obj_sep = [
                        obj_split[x : x + 7] for x in range(0, len(obj_split), 7)
                    ]
                    for line in obj_sep:
            ...
            if "gem" in item.tags:
                ...
                stats.append(
                    self.prop(
                        "Gem Help", "Place into an item socket of the right", DESC_COLOR
                    )
                )
                stats.append(
                    self.prop(
                ...
                if implicits:
                    for implicit in implicits:
                        if "{crafted}" in implicit or "(enchant)" in implicit:
                            stats.append(
                                self.prop(
                                    implicit.replace("{crafted}", "").replace(
                                        "(enchant)", ""
                                    ),
                                    "",
                                    CRAFTED,
                            ...
                            stats.append(
                                self.prop(
                                    implicit.replace("(implicit)", ""), "", PROP_COLOR
                                )
                            )
                ...
                if explicits:
                    for explicit in explicits:
                        if explicit.lower() == "corrupted":
                            ...
                            stats.append(
                                self.prop(
                                    explicit.replace("{crafted}", "").replace(
                                        " (crafted)", ""
                                    ),
                                    "",
                                    CRAFTED,
                ...
                for line in lines:
                    if len(line.split(" ")) > 7:
                        ...
                        for set_line in sep_lore:
                            stats.append(
                                self.prop("Reminder", " ".join(set_line), DESC_COLOR)
                            )
                    else:
    ...
    def render(self, poe_item):
        ...
        try:
            if self.header_font.getsize(poe_item.name) > self.header_font.getsize(
                poe_item.base
            ):
        ...
        if not isinstance(poe_item, PassiveSkill):
                ...
                if poe_item.influences:
                    ...
                    if poe_item.rarity.lower() in ["rare", "unique", "relic"]:
                        ...
                        if len(apply_influences) > 1:
                            self.namebar_right.alpha_composite(
                                apply_influences[1], (9, 18)
                            )
                        else:
                            self.namebar_right.alpha_composite(
                                apply_influences[0], (9, 18)
                            )
                    else:
                        self.namebar_left.alpha_composite(apply_influences[0], (4, 6))
                        if len(apply_influences) > 1:
                            self.namebar_right.alpha_composite(
                                apply_influences[1], (1, 6)
                            )
                        else:
                            self.namebar_right.alpha_composite(
                                apply_influences[0], (1, 6)
                            )
            except AttributeError:
                pass
            ...
            transformed_namebar = self.namebar_trans.resize(
                (
                    item.size[0] - (self.namebar_left.size[0] * 2),
        ...
        for stat in stats:
            if stat.title == "Separator":
                ...
                for attr in stat.text.keys():
                    if attr == "level":
                        d.text(
                            cur.pos, f" {attr.title()}", fill=DESC_COLOR, font=self.font
                        )
                        ...
                        d.text(
                            cur.pos, attribute_final, font=self.font, fill=DESC_COLOR
                        )
                ...
                if not isinstance(poe_item, Gem) and "shaper" in poe_item.influences:
                    ic = Image.alpha_composite(
                        self.shaper_backgrounds[poe_item.size].resize(ic.size), ic
                    )

                if not isinstance(poe_item, Gem) and "elder" in poe_item.influences:
                    ic = Image.alpha_composite(
                        self.elder_backgrounds[poe_item.size].resize(ic.size), ic
                    )
...
def parse_game_item(itemtext):
    ...
    for group in groups:
        ...
        # defense
            ...
            group[0].startswith("Quality")
            or group[0].startswith("Map Tier:")
            or group[0].startswith("Chance to Block:")
            or group[0].startswith("Armour:")
            or group[0].startswith("Evasion Rating:")
            or group[0].startswith("Energy Shield:")
        ):
            for line in group:
                if line.startswith("Quality:"):
                    pobitem["quality"] = line.replace("Quality: +", "").replace(
                        "% (augmented)", ""
                    )
                elif (
                    line.startswith("Map Tier:")
                    or line.startswith("Item Quantity:")
                    or line.startswith("Item Rarity:")
                    or line.startswith("Monster Pack Size:")
                    or line.startswith("Atlas Region:")
                ):  # map stuff
                    pobitem["implicit"].append(line)
                elif line.startswith("Quality ("):  # catalysts
                    pobitem["implicit"].append(line)
        # offense
        elif group[len(group) - 1].startswith("Attacks per Second:") or group[
            len(group) - 1
        ].startswith("Weapon Range:"):
    ...
    return {
        ...
        "base": pobitem["base"],
        "stats": pobitem["stats"],
        "rarity": pobitem["rarity"],
        "implicits": pobitem["implicit"],
        "quality": pobitem["quality"],
        "special": pobitem["special"],
...
def parse_pob_item(itemtext):
    ...
    if "implicits" in pobitem and pobitem["implicits"]:
        if pobitem["type"] == "game":
            ...
            offset = 1
        implicits = item[: pobitem["statstart_index"] + offset][
            -1 * pobitem["implicits"] :
        ]
        implicits = [implicit.replace("(implicit)", "") for implicit in implicits]

    elif (
        item[pobitem["statstart_index"] - 2].startswith("--")
        and "Item Level" not in item[pobitem["statstart_index"] - 1]
    ):
        imp_end = "None"
        for ind, stat in enumerate(item[pobitem["statstart_index"] - 1 :]):
            if stat.startswith("--"):
                if item[pobitem["statstart_index"] - 1 :][ind + 1] not in [
                    "Shaper Item",
                    "Elder Item",
                ]:
                    imp_end = ind - 1
    ...
    stat_text = [
        stat
        for stat in stat_text
        if not stat.startswith("--") and not ":" in stat and stat
    ]
...
def modify_base_stats(item):
    stats = {
        ...
        "flat armour": 0,
        "flat evasion": 0,
        "inc es": int(item.quality),
        "inc armour": int(item.quality),
        "inc evasion": int(item.quality),
        "aspd": 0,
        "fire low": 0,
        "fire max": 0,
        "fire inc": 0,
        ...
        for stat in unescape_to_list(item.implicits):
            ...
            if " to " in text and "multiplier" not in text and ":" not in text:
                if "armour" in text and isinstance(item, Armour):
                    ...
                    "block" in text
                    and "spell damage" not in text
                    and "block recovery" not in text
                    and "maximum" not in text
                ):
                    stats["block"] += int(text.split(" ")[0][:-1])
                if (
                    "damage" in text
                    and "reflect" not in text
                    and "converted" not in text
...
def _get_wiki_base(item, object_dict, cl, slot, char_api=False, thread_exc_queue=None):
    ...
    if item["rarity"].lower() in ["unique", "relic"] and char_api:
        ...
        for w in item["base"].split(" "):
            if not any(char.isdigit() for char in w):
                wl.append(w)
        try:
            wiki_base = cl.find_items(
                {"name": " ".join(wl).replace("Synthesised", "").strip()}
            )[0]
        except IndexError:
            ex = AbsentItemBaseException(f"Could not find {item['name']}")
            if thread_exc_queue:
    ...
    if (
        wiki_base.rarity.lower() not in ["unique", "relic"]
        and char_api
        or char_api is False
    ):
        if (
            wiki_base.quality == ""
            or "ring" in wiki_base.tags
            or "amulet" in wiki_base.tags
            or "belt" in wiki_base.tags
            or "quiver" in wiki_base.tags
            or "flask" in wiki_base.tags
...
def parse_pob_xml(xml: str, cl=None):
    ...
    for skill in skill_slots:
        if "slot" in skill.attrib:
            ...
            try:
                if (
                    not skill.getchildren()[0].attrib["nameSpec"]
                    in equipped["gem_groups"]
                ):
                    equipped["gem_groups"][
                        skill.getchildren()[0].attrib["nameSpec"]
                    ] = []
            except Exception:
                continue
    ...
    for socket in jewel_sockets:
        if socket.attrib["itemId"] != "0":
            ...
            parsed = parse_pob_item(
                tree.find(f'Items/Item[@id="{item_id}"]').text.replace("\t", "")
            )
    ...
    try:
        ...
        try:
            stats["total_dps"] = tree.find(
                'Build/PlayerStat[@stat="CombinedDPS"]'
            ).attrib["value"]
        except Exception:
            stats["total_dps"] = tree.find('Build/PlayerStat[@stat="TotalDPS"]').attrib[
                "value"
            ]
        ...
        try:
            ...
            skill_in_group = int(
                skill_slots[main_group - 1].attrib.get("mainActiveSkill", 1)
            )
            stats["main_skill"] = (
                skill_slots[main_group - 1]
                .getchildren()[skill_in_group - 1]
                .attrib["nameSpec"]
            )
        except Exception:
            stats["main_skill"] = " "

        stats["crit_chance"] = tree.find(
            'Build/PlayerStat[@stat="PreEffectiveCritChance"]'
        ).attrib["value"]
        stats["effective_crit_chance"] = tree.find(
            'Build/PlayerStat[@stat="CritChance"]'
        ).attrib["value"]
        stats["chance_to_hit"] = tree.find(
            'Build/PlayerStat[@stat="HitChance"]'
        ).attrib["value"]
        ...
        stats["life_regen"] = tree.find('Build/PlayerStat[@stat="LifeRegen"]').attrib[
            "value"
        ]
        stats["es"] = tree.find('Build/PlayerStat[@stat="EnergyShield"]').attrib[
            "value"
        ]
        stats["es_regen"] = tree.find(
            'Build/PlayerStat[@stat="EnergyShieldRegen"]'
        ).attrib["value"]
        try:
            stats["degen"] = tree.find('Build/PlayerStat[@stat="TotalDegen"]').attrib[
                "value"
            ]
        except AttributeError:
            stats["degen"] = "0"

        stats["evasion"] = tree.find('Build/PlayerStat[@stat="Evasion"]').attrib[
            "value"
        ]
        stats["block"] = tree.find('Build/PlayerStat[@stat="BlockChance"]').attrib[
            "value"
        ]
        stats["spell_block"] = tree.find(
            'Build/PlayerStat[@stat="SpellBlockChance"]'
        ).attrib["value"]
        stats["dodge"] = tree.find(
            'Build/PlayerStat[@stat="AttackDodgeChance"]'
        ).attrib["value"]
        stats["spell_dodge"] = tree.find(
            'Build/PlayerStat[@stat="SpellDodgeChance"]'
        ).attrib["value"]
        stats["fire_res"] = tree.find('Build/PlayerStat[@stat="FireResist"]').attrib[
            "value"
        ]
        stats["cold_res"] = tree.find('Build/PlayerStat[@stat="ColdResist"]').attrib[
            "value"
        ]
        stats["light_res"] = tree.find(
            'Build/PlayerStat[@stat="LightningResist"]'
        ).attrib["value"]
        stats["chaos_res"] = tree.find('Build/PlayerStat[@stat="ChaosResist"]').attrib[
            "value"
        ]
        try:
            stats["power_charges"] = tree.find(
                'Build/PlayerStat[@stat="PowerChargesMax"]'
            ).attrib["value"]
        except Exception:
            stats["power_charges"] = "3"
        try:
            stats["frenzy_charges"] = tree.find(
                'Build/PlayerStat[@stat="FrenzyChargesMax"]'
            ).attrib["value"]
        except Exception:
            stats["frenzy_charges"] = "3"
        try:
            stats["endurance_charges"] = tree.find(
                'Build/PlayerStat[@stat="EnduranceChargesMax"]'
            ).attrib["value"]
        except Exception:
...
def parse_poe_char_api(json, cl, items_only=False):
    rarity = {0: "Normal", 1: "Magic", 2: "Rare", 3: "Unique", 4: "Gem"}
    equipped = {}
    threads = []
    obj_dict = {}
    for item in json["items"]:
        ...
        if slot != "PassiveJewels" and "Flask" not in slot:
            t = threading.Thread(
                target=_get_wiki_base, args=(char_item, obj_dict, cl, slot, True)
            )
            threads.append(t)
            t.start()
...
def poe_skill_tree(hashes, asc: str = "None", return_keystones=False, return_asc=False):
    ...
    ascendancy_bytes = {
        "marauder": {"none": 0, "juggernaut": 1, "berserker": 2, "chieftain": 3},
        "ranger": {"none": 0, "raider": 1, "deadeye": 2, "pathfinder": 3},
        "witch": {"none": 0, "occultist": 1, "elementalist": 2, "necromancer": 3},
        "duelist": {"none": 0, "slayer": 1, "gladiator": 2, "champion": 3},
        "templar": {"none": 0, "inquisitor": 1, "hierophant": 2, "guardian": 3},
        "shadow": {"none": 0, "assassin": 1, "trickster": 2, "saboteur": 3},
        "scion": {"none": 0, "ascendant": 1},
    }
    ...
    if return_keystones and return_asc:
        return (
            f"https://www.pathofexile.com/fullscreen-passive-skill-tree/{post}",
            tree_keystones,
            ascendancy,
        )
...
def _trade_api_query(data, league, endpoint):
    ...
    resp = http.request(
        "POST",
        f"https://www.pathofexile.com/api/trade/{endpoint}/{league}",
        body=js.dumps(data).encode("utf-8"),
    ...
    entries = http.request(
        "GET",
        f'https://www.pathofexile.com/api/trade/fetch/{",".join(listing_ids[:10])}',
    )
...
def item_price(item, league):
    data = {
        "query": {"term": item, "status": {"option": "online"}},
        "sort": {"price": "asc"},
    }
