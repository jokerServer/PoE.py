...
class ClientBase:
    _dir = os.path.join(
        os.path.abspath(os.path.dirname(__file__)), "data", "valid_filters.json"
    )
    with open(_dir) as f:
        filters = json.load(f)
    ...
    def get_image_url(filename, req):
        ...
        param = {"titles": filename, "prop": "imageinfo&", "iiprop": "url"}
        dat = req(query_url, param)
        ic = dat["query"]["pages"][list(dat["query"]["pages"].keys())[0]].get(
            "imageinfo", None
        )
    ...
    def get_gems(self, where: dict, req, url):
        ...
        for gem in result_list:
            ...
            if int(gem["has percentage mana cost"]) or int(
                gem["has reservation mana cost"]
            ):
                aura = True
            else:
                aura = False
            ...
            requirements = Requirements(
                stats[1]["dexterity requirement"],
                stats[1]["strength requirement"],
                stats[1]["intelligence requirement"],
                stats[1]["level requirement"],
            )
            ...
            gem = Gem(
                ...
                gem["description"],
                gem["name"],
                gem["item class restriction"],
                gem["stat text"],
                gem["quality stat text"],
                gem["radius"],
                gem["radius description"],
                gem["radius secondary"],
                gem["radius secondary description"],
                gem["radius tertiary"],
                gem["radius tertiary description"],
                skill_icon,
                gem["skill screenshot"],
                inv_icon,
                gem["gem tags"],
                gem["tags"],
                stats,
        ...
        for item in result_list:
            ...
            if "gem" not in item["tags"].split(","):
                ...
                drops = ItemDrop(
                    item["drop enabled"],
                    item["drop level"],
                    item["drop level maximum"],
                    item["drop leagues"],
                    item["drop areas"],
                    item["drop text"],
                )
                requirements = Requirements(
                    item["required dexterity"],
                    item["required strength"],
                    item["required intelligence"],
                    item["required level"],
                )
                ...
                current_item = i(
                    ...
                    item["flavour text"],
                    item["help text"],
                    self.bool_(item["is corrupted"]),
                    self.bool_(item["is relic"]),
                    item["alternate art inventory icons"],
                    item["quality"],
                    item["implicit stat text"],
                    item["explicit stat text"],
                    # FIXME: referenced before assignment
                    item["tags"],
                    image_url,
                    influences,
